...
class Game:
    ...
    def getImage(self, point):
        ...
        if str(item) == "Part":
            ...
            count = 1
            if self._check_if_rover_on_item(item) and count > 0: # Rover on portal
                ...
                if item.linked_portal != None: # Portal is linked
                    ...
                    self._generate_new_room(new_room, Portal(item.x, item.y, new_room, item))
            else:
                return item.get_image()
...
class Room:
    ...
    def add_item(self, item):
        ...
        for x in range(15):
            for y in range(15): 
                if self.room_matrix[x][y] == 0 and count > 0:
                    self._set_object(x, y, item)
                    item.x = x
                    item.y = y
                    count -= 1
