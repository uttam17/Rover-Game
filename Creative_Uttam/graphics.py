...
# Version 1.4a
#     Button class added by Lea Wittie, 2011
#     Added anchor to options. Default is center
#     Button should really be using the tk Button but it doesnt

import time, os, sys

print("Importing mtTkinter")
...
class GraphWin(tk.Canvas):
    ...
    def getMouse(self,DEBUG=False):
        """Wait for mouse click and return Point object representing
        the click"""
        if DEBUG: print("Getting mouse now...")
        self.update()      # flush any prior clicks
        if DEBUG: print("Flushed any prior clicks...")
        self.mouseX = None
        self.mouseY = None
        while self.mouseX == None or self.mouseY == None:
            self.update()
            if self.isClosed(): raise GraphicsError("getMouse in closed window")
            if DEBUG: print("Attemptint to getMouse every 0.1 seconds...")
            time.sleep(.1) # give up thread
        x,y = self.toWorld(self.mouseX, self.mouseY)
        self.mouseX = None
        self.mouseY = None
        if DEBUG: print("Returning point at %d,%d" %(x,y))
        return Point(x,y)
...
class Scale(GraphicsObject):
   """ This will be a wrapper for a sliding scale which uses
      tkinter's Scale. """
   def __init__(self, p, minVal, maxVal, interval, initial, command=''):
   ...
   def _draw(self, canvas, options):
      ...
      self.scale = tk.Scale(frm,
                            from_=self.minVal, # min value on scale
                            to=self.maxVal, # max value on scale
                            orient='horizontal', # horiz or vert
                            length=200, # pixel length of scale
                            sliderlength=10, # pixel length of sliderbar
                            tickinterval=self.interval, # draw the number scale w/ interval
                            showvalue=0,
                            command=self.command) # show current value T/F
...
class Button(GraphicsObject):
    def __init__(self, p, text, fncn, bkcolor='LightGrey'):
        GraphicsObject.__init__(self, [])
        self.anchor = p.clone()
        self.text = text
        self.fncn = fncn
...
class OtherButton:
    """ A button with centered text. The button will be the correct size
    ...
    def __init__(self, point, text="",fncn=None):
        """ Creates a button to be located at the given point with the given text,
        and calls the given function fncn when clicked """
        self.text = Text(point, text)
        self.rectDrawn = False
        self.text.setStyle('bold')
        self.text.config["anchor"] = "nw"

    def draw(self, canvas):
        self.text.draw(canvas)
        id = self.text.id
        (x,y,x2,y2) = canvas.bbox(id)
        
        
        if self.rectDrawn == False:
            diff = 2
            x = x-diff
            y = y - diff
            x2 = x2 + diff
            y2 = y2 + diff
            self.box = Rectangle(Point(x,y), Point(x2, y2))
            self.top = Rectangle(Point(x-diff, y-diff), Point(x2+diff, y))
            self.left = Rectangle(Point(x-diff, y), Point(x-diff, y2))
            self.bottom = Rectangle(Point(x-diff, y2), Point(x2+diff, y2+diff))
            self.right = Rectangle(Point(x2, y), Point(x2+diff, y2))
            self.top.config["fill"] = "gray"
            self.left.config["fill"]= "gray"
            self.bottom.config["fill"] = "gray"
            self.right.config["fill"]= "gray"
            self.top.config["outline"] = "gray"
            self.left.config["outline"]= "gray"
            self.bottom.config["outline"] = "gray"
            self.right.config["outline"]= "gray"
            self.rectDrawn = True
    ...
    def contains(self, canvas, point):
        id = self.box.id
        (x,y,x2,y2) = canvas.bbox(id)
        if x>x2:
            temp = x2
